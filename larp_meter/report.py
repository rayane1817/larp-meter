...
def render_html(report):
    ...
    if report["sources"]:
        ...
        for u in report["sources"][:25]:
            parts.append(f"<li><a href='{_esc(u)}' rel='noreferrer noopener'>{_esc(u[:100])}</a></li>")
        parts.append("</ul>")
...
def save_all(report, output_dir, vault_path=None, html_path=None, md_path=None):
    """Write JSON (+ optional HTML/Markdown/Obsidian). Returns list of paths written."""
    written = []
    ...
    jp = output_dir / f"{ts}_{slug}.json"
    ...
    if vault_path:
        ...
        if research.is_dir():
            vp = research / f"{ts[:8]}-larp-{slug}.md"
