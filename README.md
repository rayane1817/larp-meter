...
**Belgian companies are checked against the KBO/BCE register.** Roles at an NV, BV, BVBA, SRL, SPRL, CVBA or VZW — or at an SA with a Belgian location — written in Dutch, French or English (*"Gedelegeerd bestuurder van X NV sinds 2004"*, *"Gérant de X SPRL depuis 2010"*) are looked up in Belgium's Crossroads Bank for Enterprises through its public search, which lists every entity's start date, current directors and links to other entities. A current director is confirmed; a board role dated before the company started is contradicted only when the register shows no linked predecessor entity, the legal form matches, the entity is not a non-profit (associations often run for years before registering), and a second search finds no sole-trader business under the subject's name older than the company — running a business in one's own name and incorporating it later is the honest version of the same date gap, and the register does not link the two. An NV or BV next to a Dutch location, or an SRL next to an Italian or Romanian one, is not looked up. Past directors are not shown by the public search, so a subject not listed is only a note. Keyless; if the pages change, lookups come back uncheckable rather than as findings.

**Publication claims are checked against OpenAlex.** With `--name`, a claim such as *"published over 200 papers"* or *"published extensively"* is compared with the OpenAlex author records under the subject's name — but only the records tied to the subject by an institution the profile itself names, or an ORCID it cites. Enough works corroborates the claim. Too few is shown as a note and never counted against the subject: OpenAlex often splits one researcher across several records and covers books and some fields thinly, so a shortfall is exactly what an honest, badly indexed researcher looks like. Namesakes with no stated institution, a record that lists so many institutions it probably merges several people, or a crowd of namesakes at one institution are all reported as undecided. Other people's papers (reviewed, edited, supervised) and counts in one venue (*"12 papers in Nature"*) are not read as the subject's total. It works without a key (OpenAlex allows about 100 author searches a day keyless); set `OPENALEX_API_KEY` to a free key from [openalex.org](https://openalex.org/settings/api) for ten times that. The key is sent as a header and never written to the cache.
