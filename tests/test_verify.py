...
class TestRetraction(unittest.TestCase):
    ...
    def test_end_to_end_retracted_paper_trips_the_contradiction_floor(self):
        ...
        flags.evaluate all actually carry `claim.retracted` through to a
        TRIGGERED, floor-carrying verdict rather than the field only being
        honoured by a hand-built AuditContext in test_flags.py."""
        import larp_meter.audit as audit_mod
        from larp_meter.audit import run_audit
